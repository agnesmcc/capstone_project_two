...
class Bidder {
    ...
    static async addBid(bidder, listing_id, bid) {
        const result = await db.query(
            ...
            RETURNING bidder, listing_id, bid`,
            [bidder, listing_id, bid]
        );
        return result.rows[0];
    }

    static async removeBid(bidder, listing_id) {
        const result = await db.query(
            `DELETE FROM bidders WHERE bidder = $1 AND listing_id = $2 RETURNING bidder, listing_id`,
            [bidder, listing_id]
        );
        return result.rows[0];
    }
